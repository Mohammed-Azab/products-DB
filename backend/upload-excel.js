...
async function uploadExcelToDatabase() {
  ...
  try {
    ...
    for (const sheetName of sheetNames) {
      ...
      // Determine table structure based on headers
      let insertQuery = '';
      let columns = [];
      ...
      for (let i = 0; i < dataRows.length; i++) {
        ...
        try {
          ...
          for (let j = 0; j < headers.length && j < row.length; j++) {
            ...
            switch (columnName) {
              case 'name':
                productData.name = cellValue ? cellValue.toString() : `منتج ${i + 1}`;
                break;
              case 'description':
                productData.description = cellValue ? cellValue.toString() : '';
                break;
              case 'price':
              ...
              case 'category_id':
              case 'category':
                productData.category = cellValue ? cellValue.toString() : 'عام';
                break;
              case 'sku':
                productData.sku = cellValue ? cellValue.toString() : `SKU${i + 1}`;
                break;
            }
          }
